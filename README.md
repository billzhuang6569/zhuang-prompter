...
The dev server listens on the local network by default and prints LAN URLs in the terminal. For iPad or another device, create a room on the Mac, then use the room page's `同网设备加入` LAN links or scan the player QR code; use `放大二维码` when scanning from farther away. Follow [docs/local-browser-acceptance.md](docs/local-browser-acceptance.md) for the manual pass.
