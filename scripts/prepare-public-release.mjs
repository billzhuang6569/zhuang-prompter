...
const manifest={schemaVersion:1,product:'zhuang-prompter',channel:'stable',version,publishedAt:new Date().toISOString(),sourceCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),sourceWorkingTree:true,releaseNotes:'彻底修复启动即崩溃（Cannot find module ms）：打包时用 afterPack 钩子按源码依赖树补齐安装包内缺失的传递依赖并强制校验，四端安装包均可正常启动（0.1.8 / 0.1.9 / 0.1.10 均受此问题影响，请升级到 0.1.11）。功能不变：覆盖 Apple Silicon 与 Intel(x64) 两种架构 Mac；Windows（x64 / ARM64）不变；沿用 Mac 拖拽安装、Windows 应用内更新与拍摄保护。',artifacts};
