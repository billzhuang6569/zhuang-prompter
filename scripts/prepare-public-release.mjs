...
const manifest={schemaVersion:1,product:'zhuang-prompter',channel:'stable',version,publishedAt:new Date().toISOString(),sourceCommit:execFileSync('git',['rev-parse','HEAD'],{encoding:'utf8'}).trim(),sourceWorkingTree:true,releaseNotes:'修复启动即崩溃（Cannot find module ms）：pnpm 11 下打包缺失传递依赖，四端安装包现可正常启动（0.1.8 / 0.1.9 均受影响，请升级到 0.1.10）。功能不变：覆盖 Apple Silicon 与 Intel(x64) 两种架构 Mac；Windows（x64 / ARM64）不变；沿用 Mac 拖拽安装、Windows 应用内更新与拍摄保护。',artifacts};
